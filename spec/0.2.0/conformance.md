...
MASA 0.2.0 follows the stable 0.1.0 release. Conformance evidence reports migration separately as `not_tested`, `compatible`, `incompatible`, or `not_applicable`; the reference suite currently reports `not_tested` because it validates exact-version artifacts and does not silently rewrite 0.1.0 records.
